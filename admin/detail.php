...
<!--TABLE/GRID PRESENSI -->
<div id="g_detail_tb">
<form style="display:inline;" id="detail_form">
	...
	<input class="easyui-combobox" plain="true" style="width:150px" id="id_peg"
					data-options="url:'../json/cmb_detail.php',
								method:'get',
								prompt:'Nama Pegawai',
								required: true,
								valueField:'id_peg',
...
<table class="easyui-datagrid" title="Nama: -" style="width:100%;height:320px" id="g_detail"
		data-options="singleSelect:false, url:'../json/admin_detail.php', showFooter:true,toolbar:'#g_detail_tb',
...
<script>
	function g_m_lihat_presensi(){
		if ($('#detail_form').form('validate'))
		{
			...
			var url = '../json/admin_detail.php?id_peg='+id_peg+'&bulan_c='+bulanC+'&tahun_c='+tahunC;
			$('#g_detail').datagrid('reload', url);
			$.ajax({ 
			        type: 'GET', 
			        url: '../json/admin_detail.php?id_peg='+id_peg+'&bulan_c='+bulanC+'&tahun_c='+tahunC, 
			        success: function (data) {
				        barChartData.labels.splice(0, 30); // remove chart
			            window.myBar.update();
			        	$('#g_detail').datagrid({title: 'Nama: '+data.rows[0].nama});
