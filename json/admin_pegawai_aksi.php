...
if($_GET['page']=='simpan')
{
	...
	if ($ceker==1)
	{
		echo json_encode(array('msg'=>'Kepala Sekolah sudah ada'));
	}
	else if ($ceker==2)
	{
		echo json_encode(array('msg'=>'Wakil Kepala Sekolah sudah ada'));
	}
	else
	{
		if (isset($nip))
...
if($_GET['page']=='edit')
{
	...
	if ($jabatan=='1')
	{
		$sql_cek_1 = "SELECT id_jbt, id_peg FROM pegawai WHERE id_jbt='1' AND id_peg!='$id_peg'";
		if(!$result = $db->query($sql_cek_1)){
				die('There was an error running the query [' . $db->error . ']');
				}
		$kepsek_cek=$result->num_rows;
	...
	if ($jabatan=='2')
	{
		$sql_cek_2 = "SELECT id_jbt FROM pegawai WHERE id_jbt='2' AND id_peg!='$id_peg'";
		if(!$result = $db->query($sql_cek_2)){
				die('There was an error running the query [' . $db->error . ']');
				}
		$wakasek_cek=$result->num_rows;
